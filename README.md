...
| ResNet50 @448 + 去耦旋转增广（L2r） | **99.29 ± 0.56** | **99.28 ± 0.39** | 93.15 ± 0.76 |
| ConvNeXt-Tiny @448 + 旋转增广（L6a） | 99.09 ± 0.33 | 98.36 ± 0.82 | 92.12 ± 1.06 |
| ViT-B/16-384 @448 + 旋转增广（L6b） | 98.31 ± 1.23 | 98.58 ± 0.59 | **94.26 ± 0.77** |

主要发现：朴素切分抬高 top-1 约 1.2 点、support≥5 MPC 约 3.4 点；val 与 test 在同一 epoch 上可差 ±5 点但无系统性高报；干净协议下多数模块差异落在种子噪声内，紧凑双线性明确有害，
朝向增广对有充分支撑的类别 +2.5 点，但原实现为去黑角做的放大裁剪把它抵消了，改用反射填充后兑现（L2r），且不增加参数。分析、局限与第一轮结论的
